...
func main() {
	...
	vswidthId, _ := client.CreateVSwitch(&ecs.CreateVSwitchArgs{
		ZoneId:    zoneId,
		VpcId:     vpcId,
	...
	vswitches, _, _ := client.DescribeVSwitches(&ecs.DescribeVSwitchesArgs{
		VpcId:     vpcId,
		VSwitchId: vswidthId,
	})
	for vswitches[0].Status == "Pending" {
		...
		vsws, _, _ := client.DescribeVSwitches(&ecs.DescribeVSwitchesArgs{
			VpcId:     vpcId,
			VSwitchId: vswidthId,
		})
		vswitches = vsws
	...
	instanceId, e := client.CreateInstance(&ecs.CreateInstanceArgs{
		RegionId:     region,
